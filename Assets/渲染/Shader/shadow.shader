...

{
   Properties {
		...
		_rangelight1("内圈亮度",Range(2,15)) = 8
		_rangelight2("中间亮度",Range(1,10)) = 3
	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(v2f i) : SV_Target {
				...
				float4 var_MainTexture = tex2D(_mainTexture,i.uv0);
				
				fixed shadow = SHADOW_ATTENUATION(i);
				fixed trueShadow = min(shadow ,nDir.g);

				fixed3 finalRGB =(1-trueShadow)*_shadowColor.rgb + (trueShadow * _mainColor.rgb);
		...
		Pass { // Additional Pass
				...
				fixed4 frag(v2f i) : SV_Target {
					...
					// 计算不同光源的衰减
                        ...
                        fixed atten0 = floor(atten+0.897*(1+_range1*0.07 ));
                        fixed atten1 = floor(atten+0.9*(1+_range2*0.07 ) ) -atten0 ;
					...
					return fixed4( finalRGB *_Opacity, _Opacity); // 没有再计算环境光;跟阴影的本质一样：都是与光照结果相乘
				}
				ENDCG
		}
